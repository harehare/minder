...
/// the same way `agent-hooks::engine::HookEngine` binds `__hook_arg`.
///
...
const DEFAULT_QUERY: &str = r#"read_file(path) | split(., "\n") | filter(., fn(line): is_regex_match(line, "^\\s*[-*+]\\s+\\[ \\]") end)"#;
...
/// does the reading. Mirrors `agent-hooks::engine::HookEngine`'s embedding
...
mod tests {
    ...
    fn temp_md(content: &str) -> PathBuf {
        ...
        let path =
            std::env::temp_dir().join(format!("minder-loop-test-{}-{n}.md", std::process::id()));
    ...
    fn remaining_todos_reports_a_clear_error_for_a_missing_file() {
        let err = remaining_todos(Path::new("/nonexistent/does-not-exist.md"), DEFAULT_QUERY)
            .unwrap_err();
