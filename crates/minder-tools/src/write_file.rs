...
mod tests {
    ...
    fn ctx() -> ToolContext {
        ToolContext {
            working_dir: std::env::temp_dir().join(format!("minder-write-file-test-{}", uuid::Uuid::new_v4())),
    ...
    async fn overwrites_an_existing_file() {
        ...
        tokio::fs::write(ctx.working_dir.join("a.txt"), "old").await.unwrap();

        WriteFileTool
            .execute(serde_json::json!({"path": "a.txt", "content": "new"}), &ctx)
            .await;
        let written = tokio::fs::read_to_string(ctx.working_dir.join("a.txt")).await.unwrap();
