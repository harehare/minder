...
impl HookEngine {
    ...
    pub fn load(agent_dir: &Path) -> Result<Option<Self>, HookLoadError> {
        ...
        let (search_dir, mut module_names) = if hooks_dir.is_dir() {
            ...
            for entry in std::fs::read_dir(&hooks_dir).map_err(|e| HookLoadError::Io(hooks_dir.clone(), e))? {
    ...
    async fn invoke<In: Serialize, Out: DeserializeOwned>(
        ...
        match self.engine.eval_compiled(compiled, mq_lang::null_input().into_iter()) {
            Ok(values) => {
                ...
                match serde_json::from_value::<RawDecision<Out>>(result_json) {
                    ...
                    Err(e) => self.on_error(fn_name, format!("malformed hook return value: {e}"), default, fail_mode),
                }
            }
        ...
        match self.engine.eval_compiled(compiled, mq_lang::null_input().into_iter()) {
            Ok(values) => {
                ...
                match serde_json::from_value::<RawToolCallDecision>(result_json) {
                    ...
                    Ok(RawToolCallDecision::Override { value }) => ToolCallDecision::Override(value),
                    Err(e) => self.tool_call_error(format!("malformed hook return value: {e}")),
                }
            }
            Err(err) if Self::is_not_defined(&err, "on_tool_call") => ToolCallDecision::Allow(call.clone()),
...
impl HookPort for HookEngine {
    ...
    async fn before_compact(&mut self, messages: &[Message]) -> HookDecision<()> {
        self.invoke_gate("before_compact", &messages, HookDecision::Allow(()), FailMode::Open)
            .await
    }
    ...
    async fn render_tool_result(&mut self, call: &ToolCall, outcome: &ToolExecOutcome) -> RenderDecision {
...
"#,
    ...
    use minder_core::{ContentBlock, Role, ToolResult, ToolResultContent};
...
"#;
    ...
    async fn render_tool_call_can_hide() {
        ...
        assert!(matches!(engine.render_tool_call(&call).await, RenderDecision::Hide));
    }
