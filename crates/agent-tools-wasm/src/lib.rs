...
pub enum WasmToolError {
    ...
    #[error("plugin {path} has no manifest sidecar {expected} -- a .wasm with no manifest is a hard load error, not an implicit zero-capability load")]
...
impl WasmTool {
    pub async fn load(
        ...
        let bytes = tokio::fs::read(wasm_path)
            .await
            .map_err(|e| WasmToolError::ReadWasm {
        ...
        tool.description = tool
            ...
            .await?;
        let schema_raw = tool
            .call_metadata_export(
                &mut store,
                &instance,
                &memory,
                "minder_tool_parameters_schema",
            )
            .await?;
    ...
    fn get_memory(
        ...
        instance: &Instance,
    ) -> Result<wasmtime::Memory, WasmToolError> {
        instance
...
impl WasmTool {
    async fn run(
        ...
        ctx: &ToolContext,
    ) -> Result<ToolExecOutcome, WasmToolError> {
        ...
        let raw: RawOutcome = match serde_json::from_slice(&result_bytes) {
            ...
            Err(e) => {
                return Ok(error_outcome(format!(
                    "plugin returned invalid result JSON: {e}"
                )));
            }
        };
