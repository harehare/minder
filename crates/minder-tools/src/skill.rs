...
pub fn discover_skills(agent_dir: &Path) -> Result<Vec<Skill>, SkillLoadError> {
    ...
    for dir in entries {
        ...
        let raw = std::fs::read_to_string(&skill_md).map_err(|e| SkillLoadError::Io(skill_md.clone(), e))?;
...
fn parse_skill(path: &Path, raw: &str) -> Result<Skill, SkillLoadError> {
    ...
    let description = description.ok_or_else(|| SkillLoadError::MissingField(path.to_path_buf(), "description"))?;
...
mod tests {
    ...
    fn missing_required_field_is_an_error() {
        ...
        let err = discover_skills(&agent_dir).unwrap_err();
        assert!(matches!(err, SkillLoadError::MissingField(_, "description")));
    }
    ...
    async fn loads_a_skill_body_by_name() {
        ...
        let outcome = tool.execute(serde_json::json!({"name": "example"}), &ctx()).await;
    ...
    async fn unknown_skill_name_is_an_error() {
        ...
        let outcome = tool.execute(serde_json::json!({"name": "nope"}), &ctx()).await;
        assert!(outcome.is_error);
    }
