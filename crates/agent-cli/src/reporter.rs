...
use agent_core::{HookPort, RenderDecision, Reporter, ToolCall, ToolExecOutcome};
...
impl TerminalReporter {
    ...
    fn render_diff(&self, unified: &str) -> String {
        ...
        if lines.len() > MAX_DIFF_LINES {
            rendered.push(format!(
                ...
                self.paint(
                    DIM,
                    &format!("… {} more line(s)", lines.len() - MAX_DIFF_LINES)
                )
            ));
        }
        rendered.join("\n")
    ...
    async fn render_result_decision(
        &self,
        call: &ToolCall,
...
mod tests {
    use super::*;
    use agent_core::{HookDecision, Message, ToolCallDecision, ToolResultInfo};
