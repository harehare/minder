...
async fn echo_plugin_round_trips_through_the_full_abi() {
    ...
    let outcome = tool.execute(serde_json::json!({"hello": "world"}), &ctx()).await;
...
async fn panicking_plugin_traps_are_converted_to_tool_errors_not_host_panics() {
    let tool = WasmTool::load(&fixture("panicking_plugin.wasm"), Manifest::default(), engine())
        .await
        .unwrap();
...
async fn fs_probe_plugin_is_denied_access_it_was_not_granted() {
    ...
    let tool = WasmTool::load(&fixture("fs_probe_plugin.wasm"), Manifest::default(), engine())
        .await
        .unwrap();
...
async fn fs_probe_plugin_with_unrelated_grant_is_still_denied() {
    ...
    // outside that grant.
    let dir = std::env::temp_dir().join(format!("minder-wasm-fs-grant-{}", uuid::Uuid::new_v4()));
...
async fn load_plugins_returns_empty_when_tools_dir_is_absent() {
    ...
    std::fs::create_dir_all(&dir).unwrap();
    let tools = minder_tools_wasm::load_plugins(&dir).await.unwrap();
...
async fn load_plugins_errors_on_missing_manifest_sidecar() {
    ...
    let result = minder_tools_wasm::load_plugins(&dir).await;
    assert!(result.is_err());
}

#[tokio::test]
async fn net_probe_plugin_host_fetch_enforces_the_same_ssrf_guard_as_web_fetch_tool() {
    // `host_web_fetch` calls the exact same `minder_tools::fetch` the
...
async fn net_probe_plugin_without_network_capability_fails_to_instantiate() {
    ...
    let result = WasmTool::load(&fixture("net_probe_plugin.wasm"), Manifest::default(), engine()).await;
    assert!(result.is_err());
}
