...
Run in place without installing with `cargo run -p minder-cli -- "..."`. The rest of this README
...
unrestricted by default — see [Hooks](#hooks) to add more policy before pointing minder at real
work.
...
- **stdout** — the conversation itself: assistant text, including commentary on turns that also
  call a tool.
- **stderr** — the execution trace: `● tool_name(key=value)` before each call, then either a diff
  stat line (`+N -N`) and a colorized unified diff (for `write_file`/`edit_file`, capped at 40
  lines) or a `✓`/`✗` one-line result summary.
...
state, including a `delete_file`d one. It only reaches back one turn, and deliberately isn't
...
note beyond that, so one huge pipe can't blow the context budget).
...
Set provider/model once per project instead of re-exporting env vars every session:
...
[extended thinking](https://docs.claude.com/en/docs/build-with-claude/extended-thinking) with that
...
1. Install Ollama v0.11.4+ (needed for correct gpt-oss tool-calling support):
   <https://ollama.com/download>, or `brew install ollama` (macOS) / `curl -fsSL https://ollama.com/install.sh | sh` (Linux).
2. Start the server (skip if your install already runs it as a background service): `ollama serve`.
3. Pull a gpt-oss model — `20b` needs ~16GB RAM/VRAM, `120b` needs ~65GB+ (multi-GPU/datacenter):
...
Each subagent is a directory with an `AGENT.md`: the same frontmatter shape as a [skill](#skills)
...
own system prompt. It shares the LLM provider/client, the project's [hooks](#hooks), and the live
[execution display](#quick-start) with the parent, rather than rebuilding any of those.
...
`default_policy.mq` loads right after the `agent` module and defines a baseline `on_tool_call`: it
...
The baseline is data, not an if/elif chain — `default_denylist()` is a plain array of rules
...
The [live execution display](#quick-start) is driven by two more optional hook functions, checked
before minder's own built-in formatting. Both fail **open**: a broken or undefined render function
falls back to the built-in look, since a display bug should never be able to affect what the agent
actually does.
...
Both functions only see the one call/outcome in front of them, not the conversation — a display
...
1. Query `<file>` for GFM checklist lines that are still unchecked, entirely inside `mq-lang` —
   embedded the same way as the [hooks](#hooks) engine, just pointed at a file on disk:
...
3. Otherwise the remaining items are folded into a prompt ("pick the first unfinished item,
   implement it, then check it off") and handed to `run_turn`.
4. Repeat — the item just finished no longer shows up as unchecked, so the next prompt derives
   naturally from the file's current state.
...
At that point the process keeps running: add a new `- [ ] ...` line to `TODO.md` (by hand, or have
...
- **Transient provider errors don't end the run** — rate limits, 5xx responses, and network blips
  ...
  reaching your prompt or the checklist logic.
- **The session survives a restart** — `minder loop <file>` keys its session by `<file>`'s canonical
  path and saves after every turn, so re-running the exact same command after a crash, a Ctrl-C, or
  a container restart resumes the conversation instead of starting over — at most the in-flight
  turn is lost.
...
Every push/PR to `main` runs tests, `rustfmt`, `clippy`, and `cargo-deny` (Linux; the full
...
Release](https://github.com/harehare/minder/releases) (review and publish it manually), and
publishes all crates to crates.io under the `minder-*` prefix (needs a `CARGO_REGISTRY_TOKEN`
secret). See [`release.yml`](.github/workflows/release.yml) / [`cargo-publish.yml`](.github/workflows/cargo-publish.yml).
