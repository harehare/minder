...
use agent_core::{
    AgentSession, ContentBlock, HookPort, LlmProvider, Message, ProviderError, ProviderResponse,
    Role, StopReason, Tool, ToolCall, ToolContext, ToolExecOutcome, ToolSpec, Usage,
};
...
async fn hook_script_blocks_destructive_command_through_the_real_agent_loop() {
    ...
    let hook_engine = HookEngine::load(&agent_dir)
        .unwrap()
