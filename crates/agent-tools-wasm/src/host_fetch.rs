...
//! (`agent_tools::fetch`) -- not a reimplementation.
//!
         ...
         (url_ptr, url_len, out_ptr, out_cap): (i32, i32, i32, i32)| {
            Box::new(async move {
                ...
                let (payload, is_error) =
                    match agent_tools::fetch(&client, &url, HOST_FETCH_MAX_BYTES, timeout).await {
                        Ok(result) => (
                            serde_json::json!({
                ...
                if memory
                    .write(&mut caller, out_ptr as usize, &bytes[..n])
                    .is_err()
                {
                    return -1i32;
                }
