...
impl Tool for LsTool {
    ...
    async fn execute(&self, arguments: serde_json::Value, ctx: &ToolContext) -> ToolExecOutcome {
        ...
        let result = tokio::task::spawn_blocking(move || list(&list_root, &working_dir, max_depth, show_hidden))
            .await
...
fn list(root: &Path, working_dir: &Path, max_depth: u32, show_hidden: bool) -> Result<Vec<Entry>, String> {
...
mod tests {
    ...
    async fn recursive_respects_max_depth() {
        ...
        let outcome = LsTool
            .execute(serde_json::json!({"recursive": true, "max_depth": 2}), &ctx)
            .await;
        assert!(!outcome.is_error);
    ...
    async fn show_hidden_includes_dotfiles() {
        let ctx = ctx_with_files(&[(".secret", ""), ("visible.txt", "")]).await;
        let outcome = LsTool.execute(serde_json::json!({"show_hidden": true}), &ctx).await;
        assert!(!outcome.is_error);
        assert!(outcome.content.contains(".secret"));
    }
    ...
    async fn respects_gitignore() {
        let ctx = ctx_with_files(&[(".gitignore", "ignored.txt\n"), ("ignored.txt", ""), ("kept.txt", "")]).await;
        let outcome = LsTool.execute(serde_json::json!({}), &ctx).await;
        assert!(!outcome.is_error);
